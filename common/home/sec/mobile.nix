...

{
  ...
  config = lib.mkIf config.sprrw.sec.mobile.enable {
    home.packages = with pkgs; [
      frida-tools
      (config.sprrw.sandboxing.runDockerBin { name = "apktool"; args = "${config.sprrw.sandboxing.recipes.pwd_starter} DOCKERIMG ${apktool}/bin/apktool"; })
      (config.sprrw.sandboxing.runDockerBin { name = "jadx"; args = "${config.sprrw.sandboxing.recipes.pwd_starter} DOCKERIMG ${jadx}/bin/jadx"; })
      android-tools
      (
