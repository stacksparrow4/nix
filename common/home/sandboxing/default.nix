    ...
    let
      ...
      dockerInit =
        ...
        pkgs.writeShellScript "dockerinit" ''
          ...
          export PATH="/etc/hm-package/home-path/bin:/run/current-system/sw/bin:$PATH"
          ...
          (pkgs.writeShellApplication {
            ...
            text = ''
              ...
              target=$(echo "$targets" | while read -r containerid; do
                echo "''${containerid:0:10} - $(podman exec "$containerid" ps | tail -n +2 | head -n -1 | awk '{print $4}' | awk -F/ '{print $NF}' | tr '\n' ' ')";
              ...
              podman exec -it "$target" "${dockerInit true}" bash
            '';
          })
