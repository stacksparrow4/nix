#!/usr/bin/env python3
"""
nixoverlay: A FUSE overlay filesystem for /nix/store

Two layers:
  lower = host /nix/store (read-only)
  upper = a temp directory (read-write)

Reads go to upper first, then lower.
Writes go to upper.
"""
...
class NixOverlay(Operations):
    ...
    def getattr(self, path, fh=None):
        ...
        attrs = {key: getattr(st, key) for key in (
            ...
            'st_nlink', 'st_size', 'st_uid', 'st_blocks', 'st_blksize',
        ) if hasattr(st, key)}
...
def main():
    if len(sys.argv) < 4:
        print(f"Usage: {sys.argv[0]} <lower> <upper> <mountpoint>", file=sys.stderr)
        sys.exit(1)
    ...
    FUSE(NixOverlay(lower, upper), mountpoint, foreground=True, nothreads=True,
         allow_other=False)
